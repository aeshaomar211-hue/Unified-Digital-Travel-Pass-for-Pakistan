...
export function TripPlanner({ isAuthed }: { isAuthed: boolean }) {
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[380px_1fr]">
      ...
      <div>
        {error && (
          <div className="mb-4 flex flex-col gap-2 rounded-xl border border-destructive/30 bg-destructive/10 p-4">
            <p className="text-sm font-medium text-destructive">{error}</p>
            <p className="text-xs text-muted-foreground">
              Please try again. A backup generator will provide a curated itinerary if needed.
            </p>
          </div>
        )}
